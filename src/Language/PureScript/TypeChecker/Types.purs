...
--
unifyTypes :: RefVal CheckState -> RefVal (UnifyState Type) -> Type -> Type -> Check Unit
unifyTypes chSt stRef t1 t2 = rethrowException (\x -> mkErrorStack ("Error unifying type " ++ prettyPrintType t1 ++ " with type " ++ prettyPrintType t2) Nothing <> x) $
  unifyTypes' t1 t2
  where
  unifyTypes' (TUnknown u1) (TUnknown u2) | u1 == u2 = return unit
  unifyTypes' (TUnknown u) t = substitute chSt stRef u t
  unifyTypes' t (TUnknown u) = substitute chSt stRef u t
  unifyTypes' (SaturatedTypeSynonym name args) ty = do
    ty1 <- introduceSkolemScope stRef <=< expandTypeSynonym chSt name $ args
    unifyTypes chSt stRef ty1 ty
  unifyTypes' ty s@(SaturatedTypeSynonym _ _) = unifyTypes chSt stRef s ty
  ...
  unifyTypes' (ForAll ident ty1 (Just sc)) ty2 = do
    ...
    unifyTypes chSt stRef sk ty2
  unifyTypes' (ForAll _ _ _) _ = throwException $ strMsg $ "Skolem variable scope is unspecified"
  ...
  unifyTypes' r1@(RCons _ _ _) r2 = unifyRows chSt stRef r1 r2
  unifyTypes' r1 r2@(RCons _ _ _) = unifyRows chSt stRef r1 r2
  unifyTypes' REmpty r2 = unifyRows chSt stRef REmpty r2
  unifyTypes' r1 REmpty = unifyRows chSt stRef r1 REmpty
...
-- error.
--
unifyRows :: RefVal CheckState -> RefVal (UnifyState Type) -> Type -> Type -> Check Unit
unifyRows chSt stRef r1 r2 =
  case Tuple (rowToList r1) (rowToList r2) of
    Tuple (Tuple s1 r1') (Tuple s2 r2') ->
      ...
      in do
        for_ int (uncurry (unify chSt stRef))
        unifyRows' sd1 r1' sd2 r2'
  where
  unifyRows' :: [Tuple String Type] -> Type -> [Tuple String Type] -> Type -> Check Unit
  unifyRows' [] (TUnknown u) sd r = substitute chSt stRef u (rowFromList $ Tuple sd r)
  unifyRows' sd r [] (TUnknown u) = substitute chSt stRef u (rowFromList $ Tuple sd r)
  unifyRows' ((Tuple name ty):row) r others u@(TUnknown un) = do
    occursCheck stRef un ty
    for_ row $ \(Tuple _ t) -> occursCheck stRef un t
    u' <- fresh stRef
    unify chSt stRef u $ RCons name ty u'
...
--
typesOf :: RefVal CheckState -> Maybe ModuleName -> ModuleName -> [Tuple Ident Value] -> Check [Tuple Ident (Tuple Value Type)]
typesOf chSt mainModuleName moduleName vals = do
  tys <- tidyUp <$> withSubstitution chSt (\stRef -> do
    Tuple3 es dict untypedDict <- typeDictionaryForBindingGroup stRef moduleName vals
    for es $ \e -> do
      triple@(Tuple _ (Tuple _ ty)) <- typeForBindingGroupElement chSt stRef moduleName e dict untypedDict
      ...
      if (Just moduleName == mainModuleName && fst e == Ident C.main)
        then do
          [eff, a] <- replicateM 2 (fresh stRef)
          unify chSt stRef ty $ TypeApp (TypeApp (TypeConstructor (Qualified (Just (ModuleName [ProperName "Control", ProperName "Monad", ProperName "Eff"])) (ProperName "Eff"))) eff) a
          return triple
        else return triple)

  for tys $ \(Tuple ident (Tuple val ty)) -> do
    -- Replace type class dictionary placeholders with actual dictionaries
    val' <- replaceTypeClassDictionaries chSt moduleName val
...
typeForBindingGroupElement :: RefVal CheckState
                           -> RefVal (UnifyState Type)
                           -> ModuleName
                           -> (Tuple Ident (Tuple Value (Maybe (Tuple Type Boolean))))
                           -> M.Map (Tuple ModuleName Ident) (Tuple Type NameKind)
                           -> [Tuple Ident Type]
                           -> Check (Tuple Ident (Tuple Value Type))
typeForBindingGroupElement chSt stRef moduleName (Tuple ident (Tuple val t)) dict untypedDict =
  ...
  in case t of
    ...
    Just (Tuple ty checkType) -> do
      ...
      val' <- bindNames chSt dict' $ if checkType
                                     then TypedValue true <$> check chSt stRef val ty' <*> pure ty'
                                     else return (TypedValue false val ty')
      return $ Tuple ident (Tuple val' ty')
    -- Untyped declarations
    Nothing -> do
      -- Infer the type with the new names in scope
      TypedValue _ val' ty <- bindNames chSt dict' $ infer chSt stRef val
      unify chSt stRef ty $ Unsafe.fromJust (lookup ident untypedDict)
...
--
replaceTypeClassDictionaries :: RefVal CheckState -> ModuleName -> Value -> Check Value
replaceTypeClassDictionaries chSt mn = (everywhereOnValuesTopDownM return go return).values
  where
...
entails env@(Environment envo) moduleName context = solve (sortedNubBy canonicalizeDictionary (filter filterModule context))
  ...
  solve context' (Tuple className tys) trySuperclasses =
    case sortedNubBy dictTrace (chooseSimplestDictionaries (go context' trySuperclasses className tys)) of
      [] -> throwException $ strMsg $ "No instance found for " ++ show className ++ " " ++ joinWith " " (map prettyPrintTypeAtom tys)
      [dict] -> return $ dictionaryValueToValue dict
      _ -> throwException $ strMsg $ "Overlapping instances found for " ++ show className ++ " " ++ joinWith " " (map prettyPrintTypeAtom tys)
...
expandAllTypeSynonyms :: RefVal CheckState -> Type -> Check Type
expandAllTypeSynonyms chSt = everywhereOnTypesTopDownM go
  where
  go (SaturatedTypeSynonym name args) = expandTypeSynonym chSt name args
  go other = return other
...
--
infer :: RefVal CheckState -> RefVal (UnifyState Type) -> Value -> Check Value
infer chSt stRef val = rethrowException (\x -> mkErrorStack "Error inferring type of value" (Just (ValueError val)) <> x) $ infer' val
  where
  -- |
  ...
  infer' (ArrayLiteral vals) = do
    ts <- traverse (infer chSt stRef) vals
    els <- fresh stRef
    for_ ts $ \(TypedValue _ _ t) -> unify chSt stRef els $ TypeApp tyArray t
    return $ TypedValue true (ArrayLiteral ts) els
  infer' (ObjectLiteral ps) = do
    ensureNoDuplicateProperties ps
    ts <- traverse (infer chSt stRef <<< snd) ps
  ...
  infer' (ObjectUpdate o ps) = do
    ensureNoDuplicateProperties ps
    row <- fresh stRef
    newVals <- zipWith (\(Tuple name _) t -> Tuple name t) ps <$> traverse (infer chSt stRef <<< snd) ps
    let newTys = map (\(Tuple name (TypedValue _ _ ty)) -> Tuple name ty) newVals
    oldTys <- zip (map fst ps) <$> replicateM (length ps) (fresh stRef)
    let oldTy = TypeApp tyObject $ rowFromList (Tuple oldTys row)
    o' <- TypedValue true <$> check chSt stRef o oldTy <*> pure oldTy
    return $ TypedValue true (ObjectUpdate o' newVals) $ TypeApp tyObject $ rowFromList (Tuple newTys row)
  infer' (Accessor prop val) = do
    typed@(TypedValue _ _ objTy) <- infer chSt stRef val
    propTy <- inferProperty chSt stRef objTy prop
    case propTy of
      Nothing -> do
        field <- fresh stRef
        rest <- fresh stRef
  ...
  infer' (Abs (Left arg) ret) = do
    ...
    bindLocalVariables chSt moduleName [Tuple arg ty] $ do
  ...
  infer' (App f arg) = do
    f'@(TypedValue _ _ ft) <- infer chSt stRef f
    Tuple ret app <- checkFunctionApplication chSt stRef f' ft arg Nothing
    return $ TypedValue true app ret
  infer' (Var var) = do
    Just moduleName <- getCurrentModule chSt
    ty <- introduceSkolemScope stRef <=< replaceAllTypeSynonyms chSt <=< lookupVariable chSt moduleName $ var
    case ty of
  ...
  infer' (Case vals binders) = do
    ts <- traverse (infer chSt stRef) vals
    ret <- fresh stRef
    binders' <- checkBinders chSt stRef (map (\(TypedValue _ _ t) -> t) ts) ret binders
    return $ TypedValue true (Case ts binders') ret
  infer' (IfThenElse cond th el) = do
    cond' <- check chSt stRef cond tyBoolean
    v2@(TypedValue _ _ t2) <- infer chSt stRef th
    v3@(TypedValue _ _ t3) <- infer chSt stRef el
    unify chSt stRef t2 t3
    return $ TypedValue true (IfThenElse cond' v2 v3) t2
  infer' (Let ds val) = do
    Tuple ds' val'@(TypedValue _ _ valTy) <- inferLetBinding chSt stRef [] ds val (infer chSt stRef)
...
inferLetBinding :: RefVal CheckState -> RefVal (UnifyState Type) -> [Declaration] -> [Declaration] -> Value -> (Value -> Check Value) -> Check (Tuple [Declaration] Value)
inferLetBinding _ _ seen [] ret j = Tuple seen <$> j ret
inferLetBinding chSt stRef seen (ValueDeclaration ident nameKind [] Nothing (tv@(TypedValue checkType val ty)) : rest) ret j = do
  ...
  let
    ...
    dict = M.empty
  ty' <- introduceSkolemScope stRef <=< replaceAllTypeSynonyms chSt $ ty
  TypedValue _ val' ty'' <- if checkType then bindNames chSt dict (check chSt stRef val ty') else return tv
  bindNames chSt (M.singleton (Tuple moduleName ident) (Tuple ty'' nameKind)) $ inferLetBinding chSt stRef (seen ++ [ValueDeclaration ident nameKind [] Nothing (TypedValue checkType val' ty'')]) rest ret j
inferLetBinding chSt stRef seen (ValueDeclaration ident nameKind [] Nothing val : rest) ret j = do
  valTy <- fresh stRef
  Just moduleName <- getCurrentModule chSt
  let
    dict :: M.Map (Tuple ModuleName Ident) (Tuple Type NameKind)
    ...
    dict = M.empty
  TypedValue _ val' valTy' <- bindNames chSt dict $ infer chSt stRef val
  unify chSt stRef valTy valTy'
  bindNames chSt (M.singleton (Tuple moduleName ident) (Tuple valTy' nameKind)) $ inferLetBinding chSt stRef (seen ++ [ValueDeclaration ident nameKind [] Nothing val']) rest ret j
inferLetBinding chSt stRef seen (BindingGroupDeclaration ds : rest) ret j = do
  Just moduleName <- getCurrentModule chSt
  Tuple3 es dict untypedDict <- typeDictionaryForBindingGroup stRef moduleName (map (\(Tuple3 i _ v) -> Tuple i v) ds)
  ds' <- for es $ \e -> do
    Tuple ident (Tuple val' _) <- typeForBindingGroupElement chSt stRef moduleName e dict untypedDict
    return $ Tuple3 ident LocalVariable val'
  bindNames chSt dict $ inferLetBinding chSt stRef (seen ++ [BindingGroupDeclaration ds']) rest ret j
inferLetBinding chSt stRef seen (PositionedDeclaration pos d : ds) ret j = rethrowExceptionWithPosition pos $ do
  Tuple (d' : ds') val' <- inferLetBinding chSt stRef seen (d : ds) ret j
  return $ Tuple (PositionedDeclaration pos d' : ds') val'
inferLetBinding _ _ _ _ _ _ = theImpossibleHappened "Invalid argument to inferLetBinding"
...
inferBinder chSt stRef val (StringBinder _) = unify chSt stRef val tyString *> return M.empty
inferBinder chSt stRef val (NumberBinder _) = unify chSt stRef val tyNumber *> return M.empty
inferBinder chSt stRef val (BooleanBinder _) = unify chSt stRef val tyBoolean *> return M.empty
...
inferBinder chSt stRef val (ObjectBinder props) = do
  ...
  unify chSt stRef val $ TypeApp tyObject row
  return m1
  where
  inferRowProperties :: Type -> Type -> [Tuple String Binder] -> Check (M.Map Ident Type)
  inferRowProperties nrow row [] = (unify chSt stRef nrow row) *> return M.empty
...
inferBinder chSt stRef val (ArrayBinder binders) = do
  ...
  unify chSt stRef val $ TypeApp tyArray el
  return m1
...
--
check :: RefVal CheckState -> RefVal (UnifyState Type) -> Value -> Type -> Check Value
check chSt stRef val ty = rethrowException (\x -> mkErrorStack errorMessage (Just (ValueError val)) <> x) $ check' val ty
  where
  errorMessage =
  ...
  check' val t@(ConstrainedType constraints ty) = do
    ...
    val' <- withTypeClassDictionaries chSt (zipWith (\name (Tuple className instanceTy) ->
      TypeClassDictionaryInScope { name: name, className: className, instanceTypes: instanceTy, dependencies: Nothing, ty: TCDRegular }) (map (Qualified Nothing) dictNames)
        constraints) $ check chSt stRef val ty
    return $ TypedValue true (foldr (Abs <<< Left) val' dictNames) t
  check' val (SaturatedTypeSynonym name args) = do
    ty <- introduceSkolemScope stRef <=< expandTypeSynonym chSt name $ args
    check chSt stRef val ty
  check' val u@(TUnknown _) = do
    val'@(TypedValue _ _ ty) <- infer chSt stRef val
    -- Don't unify an unknown with an inferred polytype
    (Tuple val'' ty') <- instantiatePolyTypeWithUnknowns chSt stRef val' ty
    unify chSt stRef ty' u
    return $ TypedValue true val'' ty'
  ...
  check' (ArrayLiteral vals) t@(TypeApp a ty) = do
    unify chSt stRef a tyArray
    array <- ArrayLiteral <$> for vals (\x -> check chSt stRef x ty)
    return $ TypedValue true array t
  check' (Abs (Left arg) ret) ty@(TypeApp (TypeApp t argTy) retTy) | t == tyFunction = do
    Just moduleName <- getCurrentModule chSt
    ret' <- bindLocalVariables chSt moduleName [Tuple arg argTy] $ check chSt stRef ret retTy
    return $ TypedValue true (Abs (Left arg) ret') ty
  check' (Abs (Right _) _) _ = theImpossibleHappened "Binder was not desugared"
  check' (App f arg) ret = do
    f'@(TypedValue _ _ ft) <- infer chSt stRef f
    Tuple _ app <- checkFunctionApplication chSt stRef f' ft arg (Just ret)
    return $ TypedValue true app ret
  check' v@(Var var) ty = do
    Just moduleName <- getCurrentModule chSt
    repl <- introduceSkolemScope stRef <=< replaceAllTypeSynonyms chSt <=< lookupVariable chSt moduleName $ var
    ty' <- introduceSkolemScope stRef <=< replaceAllTypeSynonyms chSt $ ty
    v' <- subsumes chSt stRef (Just v) repl ty'
    case v' of
      Nothing -> throwException $ strMsg $ "Unable to check type subsumption"
    ...
    case rowToList row of
      Tuple propsToCheck rest ->
        do let propsToRemove = map fst ps
           ...
           obj' <- check chSt stRef obj (TypeApp tyObject (rowFromList (Tuple (us ++ remainingProps) rest)))
           ps' <- checkProperties chSt stRef ps row true
           return (TypedValue true (ObjectUpdate obj' ps') t)
  check' (Accessor prop val) ty = do
    rest <- fresh stRef
    val' <- check chSt stRef val (TypeApp tyObject (RCons prop ty rest))
    return $ TypedValue true (Accessor prop val') ty
  check' (Constructor c) ty = getEnv chSt >>= \(Environment e) ->
    case M.lookup c e.dataConstructors of
      Nothing -> throwException $ strMsg $ "Constructor " ++ show c ++ " is undefined"
      Just (Tuple _ ty1) -> do
  ...
  check' val ty | containsTypeSynonyms ty = do
    ty' <- introduceSkolemScope stRef <=< expandAllTypeSynonyms chSt $ ty
    check chSt stRef val ty'
  check' (PositionedValue pos val) ty =
    rethrowExceptionWithPosition pos $ check chSt stRef val ty
...
--
checkBinders :: RefVal CheckState -> RefVal (UnifyState Type) -> [Type] -> Type -> [CaseAlternative] -> Check [CaseAlternative]
checkBinders _ _ _ _ [] = return []
checkBinders chSt stRef nvals ret (CaseAlternative { binders = binders, guard = grd, result = val } : bs) = do
  Just moduleName <- getCurrentModule chSt
  m1 <- M.unions <$> zipWithA (inferBinder chSt stRef) nvals binders
  r <- bindLocalVariables chSt moduleName (M.toList m1) $ do
    val' <- TypedValue true <$> check chSt stRef val ret <*> pure ret
    case grd of
      Nothing -> return $ mkCaseAlternative binders Nothing val'
      Just g -> do
        g' <- check chSt stRef g tyBoolean
        return $ mkCaseAlternative binders (Just g') val'
  rs <- checkBinders chSt stRef nvals ret bs
  return $ r : rs
...
--
checkProperties :: RefVal CheckState -> RefVal (UnifyState Type) -> [Tuple String Value] -> Type -> Boolean -> Check [Tuple String Value]
checkProperties chSt stRef ps row lax = case rowToList row of Tuple ts r' -> go ps ts r'
  where
  go [] [] REmpty = return []
  go [] [] u@(TUnknown _) = do unify chSt stRef u REmpty
                               return []
  ...
  go ((Tuple p v):ps') [] u@(TUnknown _) = do
    v'@(TypedValue _ _ ty) <- infer chSt stRef v
    rest <- fresh stRef
    unify chSt stRef u $ RCons p ty rest
    ps'' <- go ps' [] rest
    return $ (Tuple p v') : ps''
  go ((Tuple p v):ps') ts r =
    case lookup p ts of
      Nothing -> do
        v'@(TypedValue _ _ ty) <- infer chSt stRef v
        rest <- fresh stRef
        unify chSt stRef r $ RCons p ty rest
        ps'' <- go ps' ts rest
        return $ (Tuple p v') : ps''
      Just ty -> do
        v' <- check chSt stRef v ty
...
--
checkFunctionApplication :: RefVal CheckState -> RefVal (UnifyState Type) -> Value -> Type -> Value -> Maybe Type -> Check (Tuple Type Value)
checkFunctionApplication chSt stRef fn fnTy arg ret = rethrowException (\x -> mkErrorStack errorMessage (Just (ValueError fn)) <> x) $ checkFunctionApplication' fn fnTy arg ret
  where
  ...
  checkFunctionApplication' fn (ForAll ident ty _) arg ret = do
    replaced <- replaceVarWithUnknown stRef ident ty
    checkFunctionApplication chSt stRef fn replaced arg ret
  checkFunctionApplication' fn u@(TUnknown _) arg ret = do
    arg' <- do
      TypedValue _ arg' t <- infer chSt stRef arg
      Tuple arg'' t' <- instantiatePolyTypeWithUnknowns chSt stRef arg' t
      return $ TypedValue true arg'' t'
    let ty = (\(TypedValue _ _ t) -> t) arg'
    ret' <- maybe (fresh stRef) return ret
    unify chSt stRef u $ function ty ret'
    return $ Tuple ret' (App fn arg')
  checkFunctionApplication' fn (SaturatedTypeSynonym name tyArgs) arg ret = do
    ty <- introduceSkolemScope stRef <=< expandTypeSynonym chSt name $ tyArgs
    checkFunctionApplication chSt stRef fn ty arg ret
  ...
  checkFunctionApplication' _ fnTy arg _ = throwException $ strMsg $ "Cannot apply a function of type "
    ++ prettyPrintType fnTy
    ++ " to argument " ++ prettyPrintValue arg
...
subsumes chSt stRef val ty1 ty2 = rethrowException (\x -> mkErrorStack errorMessage (ValueError <$> val) <> x) $ subsumes' val ty1 ty2
  ...
  subsumes' val (TypeApp f1 r1) (TypeApp f2 r2) | f1 == tyObject && f2 == tyObject =
    case Tuple (rowToList r1) (rowToList r2) of
      Tuple (Tuple ts1 r1') (Tuple ts2 r2') ->
        ...
        in do
          ...
          return val
    where
    go [] ts2 r1' r2' = unify chSt stRef r1' $ rowFromList (Tuple ts2 r2')
    ...
    go ((Tuple p1 ty1) : ts1) ((Tuple p2 ty2) : ts2) r1' r2' = case unit of
        ...
        _ | p1 < p2 -> do
          rest <- fresh stRef
          unify chSt stRef r2' $ RCons p1 ty1 rest
          go ts1 ((Tuple p2 ty2) : ts2) r1' rest
        _ -> do
          rest <- fresh stRef
          unify chSt stRef r1' $ RCons p2 ty2 rest
